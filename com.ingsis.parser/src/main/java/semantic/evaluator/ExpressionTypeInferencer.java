...
public class ExpressionTypeInferencer {
    ...
    private Result<DataType> inferIdentifierType(IdentifierNode id, SemanticEnvironment env) {
        ...
        if (symbolOpt.isEmpty()) {
            return Result.failure("Variable '" + id.name() + "' is not declared (line " + id.line() + ")");
        }
    ...
    private Result<DataType> inferOperatorType(OperatorNode op, SemanticEnvironment env) {
        ...
        return switch (op.operatorType()) {
            case PLUS -> (left == DataType.STRING || right == DataType.STRING)
                    ? Result.success(DataType.STRING)
                    : (left == DataType.NUMBER && right == DataType.NUMBER)
                    ? Result.success(DataType.NUMBER)
                    : Result.failure("Incompatible operands for '+' operator at line " + op.line());
            case MINUS, STAR, SLASH -> (left == DataType.NUMBER && right == DataType.NUMBER)
                    ? Result.success(DataType.NUMBER)
                    : Result.failure("Numeric operator requires number operands at line " + op.line());
