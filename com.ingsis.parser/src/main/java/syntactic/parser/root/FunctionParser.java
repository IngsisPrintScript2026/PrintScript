    ...
    public Result<IterationStep<CallFunctionNode>> parse(TokenStream stream) {
        ...
        if (!idResult.isCorrect()) {
            return Result.failure(
                    ((IncorrectResult<IterationStep<IdentifierNode>>) idResult).error());
        }
        IterationStep<IdentifierNode> idStep =
                ((CorrectResult<IterationStep<IdentifierNode>>) idResult).value();
        return parseArguments(idStep.value(), (TokenStream) idStep.next());
    }

    private Result<IterationStep<CallFunctionNode>> parseArguments(
            IdentifierNode idNode, TokenStream stream) {
        Result<IterationStep<List<ExpressionNode>>> argsResult =
        ...
        if (!argsResult.isCorrect()) {
            return Result.failure(
                    ((IncorrectResult<IterationStep<List<ExpressionNode>>>) argsResult).error());
        }

        IterationStep<List<ExpressionNode>> argsStep =
                ((CorrectResult<IterationStep<List<ExpressionNode>>>) argsResult).value();
