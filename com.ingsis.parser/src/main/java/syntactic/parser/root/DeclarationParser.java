                ...
                yield switch (identifierParser.parse(currentStream)) {
                    case CorrectResult<IterationStep<IdentifierNode>>(
                            ...
                            parseColon(
                                    ...
                                    declType,
                                    idStep.value(),
                                    (TokenStream) idStep.next());
                    case IncorrectResult<IterationStep<IdentifierNode>>(String err) ->
                            Result.failure(err);
                };
            }
    ...
    private Result<IterationStep<DeclarationKeywordNode>> parseStrategy(
        ...
        if (symbolOpt.isPresent() && symbolStrategies.containsKey(symbolOpt.get())) {
            ...
            return strategy.parse(
                    keywordToken, declType, identifier, declaredType, stream, expressionParser);
        }
