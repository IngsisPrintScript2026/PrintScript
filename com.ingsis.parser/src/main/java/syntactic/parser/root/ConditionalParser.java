...
public class ConditionalParser implements Parser<IfKeywordNode> {
    ...
    public ConditionalParser(Parser<ExpressionNode> conditionParser, Parser<Node> statementParser) {
        this(conditionParser, statementParser, List.of(new WithElseStrategy(), new WithoutElseStrategy()));
    }
    ...
    public Result<IterationStep<IfKeywordNode>> parse(TokenStream stream) {
        ...
        IterationStep<Token> lParenStep = ((CorrectResult<IterationStep<Token>>) lParenResult).value();
    ...
    private Result<IterationStep<IfKeywordNode>> parseCondition(Token ifToken, TokenStream stream) {
        ...
        if (!condResult.isCorrect()) {
            return Result.failure(((IncorrectResult<IterationStep<ExpressionNode>>) condResult).error());
        }

        IterationStep<ExpressionNode> condStep = ((CorrectResult<IterationStep<ExpressionNode>>) condResult).value();
        ...
        IterationStep<Token> rParenStep = ((CorrectResult<IterationStep<Token>>) rParenResult).value();
        return parseThenBody(ifToken, condStep.value(), (TokenStream) rParenStep.next());
    }

    private Result<IterationStep<IfKeywordNode>> parseThenBody(Token ifToken, ExpressionNode condition, TokenStream stream) {
        Result<IterationStep<List<Node>>> blockResult = BlockParserUtils.parseBlock(
                stream, statementParser, SymbolType.LBRACE, SymbolType.RBRACE);

        if (!blockResult.isCorrect()) {
            return Result.failure(((IncorrectResult<IterationStep<List<Node>>>) blockResult).error());
        }

        IterationStep<List<Node>> blockStep = ((CorrectResult<IterationStep<List<Node>>>) blockResult).value();
        return parseElse(ifToken, condition, blockStep.value(), (TokenStream) blockStep.next());
    }

    private Result<IterationStep<IfKeywordNode>> parseElse(Token ifToken, ExpressionNode condition, List<Node> thenBody, TokenStream stream) {
