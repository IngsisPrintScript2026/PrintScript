...
public class WithElseStrategy implements ConditionalElseStrategy {
    ...
    public Result<IterationStep<IfKeywordNode>> parseElse(
        ...
        Result<IterationStep<List<Node>>> elseBlockResult = BlockParserUtils.parseBlock(
                postElseTokenStream, statementParser, SymbolType.LBRACE, SymbolType.RBRACE);

        if (!elseBlockResult.isCorrect()) {
            return Result.failure(((IncorrectResult<IterationStep<List<Node>>>) elseBlockResult).error());
        }

        IterationStep<List<Node>> elseBlockStep = ((CorrectResult<IterationStep<List<Node>>>) elseBlockResult).value();
