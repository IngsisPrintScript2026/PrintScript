...
public class CallFunctionScaHandler implements ScaNodeHandler<CallFunctionNode> {
    ...
    @Override
    public List<String> check(CallFunctionNode call, SemanticEnvironment env, ScaContext context, ASTSca sca) {
        List<String> violations = new ArrayList<>();
        String functionName = call.identifierNode().name();

        boolean checkPrintln = "println".equalsIgnoreCase(functionName) && context.mandatoryLiteralOrIdentifierInPrintln();
        boolean checkReadInput = "readInput".equalsIgnoreCase(functionName) && context.mandatoryLiteralOrIdentifierInReadInput();
            ...
            for (int i = 0; i < args.size(); i++) {
                ...
                if (!(arg instanceof LiteralNode) && !(arg instanceof IdentifierNode)) {
                    violations.add(String.format(
                            "Function '%s' argument at index %d must be a literal or variable, found expression at line %d, column %d",
