...
public class AssignParser implements Parser<AssignNode> {
    ...
    public Result<IterationStep<AssignNode>> parse(TokenStream stream) {
        ...
        IterationStep<IdentifierNode> idStep = ((CorrectResult<IterationStep<IdentifierNode>>) idRes).value();
        ...
        if (!exprRes.isCorrect()) {
            return Result.failure(((IncorrectResult<IterationStep<ExpressionNode>>) exprRes).error());
        }

        IterationStep<ExpressionNode> exprStep = ((CorrectResult<IterationStep<ExpressionNode>>) exprRes).value();
